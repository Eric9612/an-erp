		...
		<div class="main-panel-content">
			<div class="operate-panel">
				...
				<div class="button-list pull-right">
					<Button  type="primary" @click="$router.push('/production-return/add')">{{ $t('common.ADD') }}</Button>
				</div>
    ...
    export default {
                    ...
                    { title: this.$t('field.OPERATE'), key: 'action', render: (h, params) => {
                            return h('div', [
                                h('Button', {
                                    ...
                                    on: { click: () => {
                                            this.$router.push('/production-return/' + params.row[this.vm.identity])
                                        }}
        ...
        methods: {
            ...
            async search() {
                ...
                let result = await productionReturnService.searchBill(JSON.stringify(this.searchKey))
                console.log(result)
                if (result.status === 200) {
                    ...
                    items.forEach((item) => {
                        ...
                        if (item.returnReason === 1) {
                            item.returnReason = this.$t('field.RETURN_REASON.REASON1')
                        }
            ...
            remove(selectItems) {
                ...
                this.$Modal.confirm({
                    ...
                    onOk: async () => {
                        let result = await productionReturnService.deletebills(idList)
                        if (result.status === 200) {
