...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async getById() {
                let result = await productionReturnService.getBill(this.bill.billId);
                console.log(result)
                ...
                this.$Modal.confirm({
                    ...
                    onOk: async () => {
                        ...
                        if (!isNaN(Number(this.$route.params.id))) {
                            ...
                            let result = await productionReturnService.deletematerials(this.bill.billId,tmpidList)
                            if (result.status != 200) {
                            ...
                            for(j in idList){
                                if(this.all_mate_data[i].materialId==idList[j]){
                                    if (this.$route.params.id === 'add') {
                                        var tmpj=idList[j];
                                        console.log(tmpj);
                                        let result = await productionReturnService.deleteitem(tmpj);
                                        if (result.status != 200) {
                                            this.$Message.error("error")
                                        }
                                    }
            ...
            async savemeta() {
                if (!isNaN(Number(this.$route.params.id))) {
                    ...
                    console.log("************")
                    let result = await productionReturnService.changematerials(this.bill.billId,this.allmetainfo[this.meta.id].materialId,count,this.meta.remark);
                    console.log(result);
                    ...
                    let result = await productionReturnService.addmaterial(this.allmetainfo[this.meta.id].materialId,this.meta.count,this.meta.remark);
                    if(result.data=="success"){
                        console.log(result);
                        var new_data=[{}];
                        new_data[0].materialId=this.allmetainfo[this.meta.id].materialId;
            ...
            async savebill(){
                if (this.$route.params.id === 'add') {
                    ...
                    let result = await productionReturnService.addBill(this.bill.billNo,this.bill.remark,2);
                    if (result.status != 200) {
                        this.$Message.error("error")
                    }
                }
            ...
            async searchAllBillMaterials() {
                ...
                let result = await productionReturnService.getAllBillMaterials(this.bill.billId);
                console.log('searchAllBillMaterials');
            ...
            statusto0(){
                this.status=0;
                let result = productionReturnService.changestatus(this.bill.billId,0);
                console.log(result);
            },
            statusto1(){
                this.status=1;
                let result = productionReturnService.changestatus(this.bill.billId,1);
                console.log(result);
            }
        },
        created() {
