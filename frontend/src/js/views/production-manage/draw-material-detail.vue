        ...
        <div class="main-panel-content2">
            <div class="panel-container">
                <i-form ref="formValidate" :model="bill" :rules="rules" :label-width="90" inline>
                    ...
                    <div class="chief-panel">
                        <div class="panel-body">
                            <form-item :label="$t('field.DRAW_MATERIAL.DRAW_REASON')"><i-input v-model="bill.drawReason" disabled></i-input></form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async getById() {
                let result = await productionDrawService.getBill(this.bill.billId);
                console.log(result)
                ...
                this.$Modal.confirm({
                    ...
                    onOk: async () => {
                        ...
                        if (!isNaN(Number(this.$route.params.id))) {
                            ...
                            let result = await productionDrawService.deletematerials(this.bill.billId,tmpidList)
                            if (result.status != 200) {
                            ...
                            for(j in idList){
                                if(this.all_mate_data[i].materialId==idList[j]){
                                    if (this.$route.params.id === 'add') {
                                        var tmpj=idList[j];
                                        console.log(tmpj);
                                        let result = await productionDrawService.deleteitem(tmpj);
                                        if (result.status != 200) {
                                            this.$Message.error("error")
                                        }
                                    }
            ...
            async savemeta() {
                if (!isNaN(Number(this.$route.params.id))) {
                    ...
                    console.log("************")
                    let result = await productionDrawService.changematerials(this.bill.billId,this.allmetainfo[this.meta.id].materialId,count,this.meta.remark);
                    console.log(result);
                    ...
                    let result = await productionDrawService.addmaterial(this.allmetainfo[this.meta.id].materialId,this.meta.count,this.meta.remark);
                    if(result.data=="success"){
                        console.log(result);
                        var new_data=[{}];
                        new_data[0].materialId=this.allmetainfo[this.meta.id].materialId;
            ...
            async savebill(){
                if (this.$route.params.id === 'add') {
                    ...
                    let result = await productionDrawService.addBill(this.bill.billNo,this.bill.remark,2);
                    if (result.status != 200) {
                        this.$Message.error("error")
                    }
                }
            ...
            async searchAllBillMaterials() {
                ...
                let result = await productionDrawService.getAllBillMaterials(this.bill.billId);
                console.log('searchAllBillMaterials');
            ...
            statusto0(){
                this.status=0;
                let result = productionDrawService.changestatus(this.bill.billId,0);
                console.log(result);
            },
            statusto1(){
                this.status=1;
                let result = productionDrawService.changestatus(this.bill.billId,1);
                console.log(result);
            }
        },
        created() {
